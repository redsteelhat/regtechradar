"""BIS (Bank for International Settlements) crawler — page_list + generic extraction."""
...
class BISSource(AbstractSource):
    """BIS crawler — /bcbs/publications and /cpmi/publications list pages."""
    ...
    async def fetch(self) -> list[dict[str, Any]]:
        ...
        async with httpx.AsyncClient(headers=_headers(), timeout=30.0) as client:
            ...
            for item_url in links[:MAX_ITEMS_PER_TARGET]:
                if "/bcbs/" not in item_url and "/cpmi/" not in item_url:
                    continue
                ...
                title = title_el.get_text(strip=True) if title_el else "BIS update"
