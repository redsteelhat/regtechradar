"""FCA (Financial Conduct Authority, UK) crawler — page_list + generic extraction."""
...
class FCASource(AbstractSource):
    """FCA crawler — /news and /publications list pages."""
    ...
    async def fetch(self) -> list[dict[str, Any]]:
        ...
        async with httpx.AsyncClient(headers=_headers(), timeout=30.0) as client:
            ...
            for item_url in links[:MAX_ITEMS_PER_TARGET]:
                if "/news" not in item_url and "/publications" not in item_url:
                    continue
                ...
                title = title_el.get_text(strip=True) if title_el else "FCA update"
                body_el = soup.select_one("main") or soup.select_one("article") or soup.select_one("[role='main']")
