"""FinCEN (Financial Crimes Enforcement Network, US) crawler — page_list + generic extraction."""
...
class FinCENSource(AbstractSource):
    """FinCEN crawler — /news-room list page."""
    ...
    async def fetch(self) -> list[dict[str, Any]]:
        """Fetch from page_list target: list page -> article links -> content."""
        ...
        async with httpx.AsyncClient(headers=_headers(), timeout=30.0) as client:
            ...
            for item_url in links[:MAX_ITEMS_PER_TARGET]:
                if "/news-room" not in item_url and "fincen.gov" not in item_url:
                    continue
                ...
                title = title_el.get_text(strip=True) if title_el else "FinCEN update"
