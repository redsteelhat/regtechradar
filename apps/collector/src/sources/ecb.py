"""ECB (European Central Bank) crawler — page_list + generic extraction."""
...
class ECBSource(AbstractSource):
    """ECB crawler — /press/pr and /pub/ list pages."""
    ...
    async def fetch(self) -> list[dict[str, Any]]:
        ...
        async with httpx.AsyncClient(headers=_headers(), timeout=30.0) as client:
            ...
            for item_url in links[:MAX_ITEMS_PER_TARGET]:
                if "/press/" not in item_url and "/pub/" not in item_url:
                    continue
                ...
                title = title_el.get_text(strip=True) if title_el else "ECB update"
